...
extension Windows.Kernel.File.Flush {
    ...
    /// (`flush(_:)` taking `Windows.Kernel.Descriptor`) delegates to this raw SPI
    ...
    /// - Throws: `Windows.Kernel.File.Flush.Error` on failure.
    @_spi(Syscall)
    public static func flush(_ handle: UInt) throws(Windows.Kernel.File.Flush.Error) {
...
extension Windows.Kernel.File.Flush {
    ...
    /// `Windows.Kernel.File.Flush+CrossPlatform.Windows.swift` defines `data(_:)`
    /// and `directory(path:)` only — no method-name overlap with this
    /// file's `flush(_:)`/`flushData(_:)`. Per [PLAT-ARCH-008e] empty-tier
    /// exception, the unifier `Windows.Kernel.File.Flush.flush(_:)` is inherited
    /// from this L2 form via `Windows.Kernel == Kernel` namespace identity.
    ///
    /// - Parameter descriptor: The file descriptor to flush.
    /// - Throws: `Windows.Kernel.File.Flush.Error` on failure.
    public static func flush(_ descriptor: Windows.Kernel.Descriptor) throws(Windows.Kernel.File.Flush.Error) {
    ...
    /// - Parameter descriptor: The file descriptor to flush.
    /// - Throws: `Windows.Kernel.File.Flush.Error` on failure.
    public static func flushData(_ descriptor: Windows.Kernel.Descriptor) throws(Windows.Kernel.File.Flush.Error) {
