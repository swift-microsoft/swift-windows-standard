...
extension Windows.Kernel.Memory.Allocation {
    ...
    public static func free(
        ...
        guard unsafe VirtualFree(addr.mutablePointer, 0, DWORD(MEM_RELEASE)) else {
            throw .free(Windows.Kernel.Error.captureLastError())
        }
    }
...
extension Windows.Kernel.Memory.Allocation {
    ...
    public enum Error: Swift.Error, Sendable, Equatable {
        ...
        case allocate(Kernel.Error.Code)

        /// Free failed with platform error.
        case free(Kernel.Error.Code)
...
extension Windows.Kernel.Memory.Allocation.Error {
    ...
    internal static func current() -> Self {
        .allocate(Windows.Kernel.Error.captureLastError())
    }
}
