...
extension Windows.Loader.Library {
    ...
    public struct Flags: OptionSet, Sendable {
        ...
        /// and termination.
        public static let dontResolveDllReferences = Flags(rawValue: DWORD(DONT_RESOLVE_DLL_REFERENCES))

        /// The system does not check AppLocker rules or apply Software
        /// Restriction Policies for the DLL.
        public static let loadIgnoreCodeAuthzLevel = Flags(rawValue: DWORD(LOAD_IGNORE_CODE_AUTHZ_LEVEL))

        /// If this value is used, the system maps the file into the calling
        /// process's virtual address space as if it were a data file.
        public static let loadLibraryAsDatafile = Flags(rawValue: DWORD(LOAD_LIBRARY_AS_DATAFILE))
        ...
        public static let loadLibraryAsDatafileExclusive = Flags(rawValue: DWORD(LOAD_LIBRARY_AS_DATAFILE_EXCLUSIVE))

        /// If this value is used, the system maps the file into the process's
        /// virtual address space as an image file.
        public static let loadLibraryAsImageResource = Flags(rawValue: DWORD(LOAD_LIBRARY_AS_IMAGE_RESOURCE))
        ...
        public static let loadWithAlteredSearchPath = Flags(rawValue: DWORD(LOAD_WITH_ALTERED_SEARCH_PATH))
    }
}
